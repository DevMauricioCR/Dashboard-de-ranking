...
.pev-card-title{ display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; padding-bottom:8px; border-bottom:1px solid rgba(255,255,255,0.06); }
...
.pev-kpi{ padding:12px 22px; margin-bottom:8px; background:linear-gradient(135deg, rgba(0,255,214,0.08), rgba(255,46,126,0.04)); border-color:rgba(0,255,214,0.3); }
...
.pev-table th{ text-align:left; font-size:11.5px; text-transform:uppercase; letter-spacing:0.06em; color:var(--muted); font-weight:700; padding:0 14px 6px 14px; }
.pev-table th.num{ text-align:right; }
.pev-table td{ padding:7px 14px; font-size:14px; border-top:1px solid rgba(255,255,255,0.06); vertical-align:top; color:var(--text); }
...
.pev-count-big{ font-size:19px; font-weight:800; color:var(--cyan); }
    ...
    <div style={{ height: '100%', overflow: 'auto' }}>
      ...
      <div className="pev-card">
        ...
        <table className="pev-table">
          ...
          <thead>
            <tr>
              <th style={{ fontSize: 11.5 }}>EVENTO</th>
              <th className="num" style={{ fontSize: 11.5 }}>PARTICIPANTES</th>
            </tr>
          </thead>
          <tbody>
            {eventos.map((ev, i) => (
              <tr key={`${ev.hoja}-${i}`}>
                <td>
                  <div className="pev-evento-name" style={{ fontSize: 14.5 }}>{ev.hoja}</div>
                </td>
                <td className="num">
                  <span className="pev-count-big" style={{ fontSize: 19 }}>{ev.participantes}</span>
                </td>
              </tr>
            ))}
          </tbody>
